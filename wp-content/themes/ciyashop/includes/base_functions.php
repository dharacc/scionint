...
function ciyashop_shop_page_sidebar() {
	...
	if ( ! isset( $ciyashop_options ) || empty( $ciyashop_options ) ) {
		return 'left';
	}
...
function ciyashop_product_cat_search( $query ) {
	...
	if ( $query->is_main_query() && ! empty( $wp_the_query->query_vars['wc_query'] ) && isset( $_GET['search_category'] ) && ! empty( $_GET['search_category'] ) ) {
...
/**
 * Ciyashop remove pre get posts
 */
function ciyashop_remove_pre_get_posts() {
	global $wp_the_query;
	if ( ! empty( $wp_the_query->query_vars['wc_query'] ) && isset( $_GET['search_category'] ) && ! empty( $_GET['search_category'] ) ) {
...
function ciyashop_topbar_enable() {
	...
	$topbar_enable_stat = ( isset( $ciyashop_options['topbar_enable'] ) && ! empty( $ciyashop_options['topbar_enable'] ) ) ? $ciyashop_options['topbar_enable'] : false;
...
function ciyashop_topbar_mobile_enable() {
	...
	$topbar_mobile_enable_stat = ( isset( $ciyashop_options['topbar_mobile_enable'] ) && ! empty( $ciyashop_options['topbar_mobile_enable'] ) ) ? $ciyashop_options['topbar_mobile_enable'] : true;
	...
	function ciyashop_breadcrumbs( $class = '' ) {
		...
		if ( is_archive() && ! is_tax() && ! is_category() && ! is_tag() && ! is_author() ) {
			...
			foreach($brandterms as $termv) 
				...
				if(!empty($taxname))
				{
				   $cat_display .= '<li class="item-cat"><span class="item-element"><a href="">' . $taxname . '</a></span></li>';
				}
			}
			...
			if ( ! empty( $last_category ) ) {
				...
				$gender=get_post_meta( $post->ID, 'scion_gender', true );
				$size=get_post_meta( $post->ID, 'scion_size', true );
				$variant=get_post_meta( $post->ID, 'scion_variant', true );
				if(!empty($gender))
				{
					$output .= '<li class="current-item item-' . $post->ID . '"><span class="item-element">' . $gender . '</span></li>';
				}
				if(!empty($size))
				{
					$output .= '<li class="current-item item-' . $post->ID . '"><span class="item-element">' . $size . '</span></li>';
				}
				if(!empty($variant))
				{
					$output .= '<li class="current-item item-' . $post->ID . '"><span class="item-element">' . $variant . '</span></li>';
				}
