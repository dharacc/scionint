...
//	wp_reset_query();
    ...
    wp_reset_query();
    echo '<div class="myrelatedpro" style="padding-top:50px;"> <h4>Related Products from '.$brand_display.'</h4>';
...
//print_r($the_query);
   // woocommerce_output_related_products();
 //   wp_reset_query();
		echo do_shortcode('[yith_wcbr_brand_product pagination="no" per_page="4" cols="4" product_type="all" orderby="rand" order="DESC" hide_free="no" show_hidden="no" show_brand_box="no" brand="'.$brand_display.'"]');
		echo '</div>';
    $args1 = array(
        'post_type' => 'product',
...
if('BOUGHT'==1){
		echo '<div class="myrelatedpro" style="padding-top:50px;"> <h4>Other Customers Bought</h4>';
		woocommerce_output_related_products();
		echo '</div>';
}
	// }
...
function ciyashop_woocommerce_output_related_products_args( $args ) {
	...
	$related_products_per_page = isset( $ciyashop_options['related_products_per_page'] ) && ! empty( $ciyashop_options['related_products_per_page'] ) ? $ciyashop_options['related_products_per_page'] : 8;
...
if ( ! function_exists( 'ciyashop_new_product_tab_content' ) ) {
	...
	function ciyashop_new_product_tab_content() {
		...
		if ( $custom_tab_content ) {
			echo get_post_meta( get_the_ID(), 'custom_tab_content', true ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
}
					...
					<div class="col-lg-7">
						...
						<div class="ciyashop-sticky-btn-cart">
							<div class="wishlist-compare-button">
								 ...
								 * @ignore
								 */
								if ( in_array( 'yith-woocommerce-wishlist/init.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ), true ) || in_array( 'yith-woocommerce-wishlist-premium/init.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ), true ) ) {
								 ...
								 * @ignore
								 */
								if ( in_array( 'yith-woocommerce-compare/init.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ), true ) ) {
