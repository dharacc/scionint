...
<div class="search_form-wrap">
	<div class="<?php echo esc_attr( $search_form_classes ); ?>">
		<form class="search-form" role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>">
			<?php
			if ( ( isset( $ciyashop_options['search_content_type'] ) && ( 'post' === $ciyashop_options['search_content_type'] || 'product' === $ciyashop_options['search_content_type'] ) )
				...
				<div class="search_form-category-wrap">
					...
					if ( 'product' === $ciyashop_options['search_content_type'] && isset( $_GET['product_cat'] ) && ! empty( $_GET['product_cat'] ) ) {
					...
					if ( 'product' === $ciyashop_options['search_content_type'] ) {
					...
					$args = array(
						'hide_empty'       => false,
						'hierarchical'     => 1,
						'taxonomy'         => ( isset( $ciyashop_options['search_content_type'] ) && 'product' === $ciyashop_options['search_content_type'] ) ? 'product_cat' : 'category',
						'pad_counts'       => false,
						'name'             => ( isset( $ciyashop_options['search_content_type'] ) && 'product' === $ciyashop_options['search_content_type'] ) ? 'product_cat' : 'search_category',
						'class'            => 'search_form-category',
						'id'               => 'search_category_' . $search_category_sr,
						'show_option_all'  => esc_html__( 'All Categories', 'ciyashop' ),
						'selected'         => $selected_cat,
						'value_field'      => ( isset( $ciyashop_options['search_content_type'] ) && 'product' === $ciyashop_options['search_content_type'] ) ? 'slug' : 'term_id',
						'show_option_none' => false,
						'hide_if_empty'    => false,
						'exclude'          => $exclude,
					);

					if ( isset( $ciyashop_options['search_content_type'] ) && 'product' === $ciyashop_options['search_content_type'] ) {
			...
			<div class="search_form-input-wrap">
				<?php
				if ( isset( $ciyashop_options['search_content_type'] ) && 'all' !== $ciyashop_options['search_content_type'] ) {
					?>
	...
	<?php
	if ( ( isset( $ciyashop_options['show_search_keywords'] ) && $ciyashop_options['show_search_keywords'] ) && 'default' !== $ciyashop_options['header_type'] && 'product' === $ciyashop_options['search_content_type'] ) {
