...
<table class="shop_table cart" id="yith-ywrac-table-list" cellspacing="0">
	...
	<?php foreach ( $cart_content['cart'] as $cart_item_key => $cart_item ):
		...
		if( 'incl' == $tax_display_cart ){
			...
			$price_subtotal = wc_price( $price, array( 'currency'=> $currency, 'ex_tax_label'=> true ) );
			$price_total = wc_price( $price * $qty, array( 'currency'=> $currency, 'ex_tax_label'=> true ) );
		}
		?>
        <tr class="cart_item">
    ...
    <tr>
		...
		<?php $product_subtotal = wc_price( $total, array( 'currency' => $currency ) ) . ' <small class="tax_label">' . WC()->countries->ex_tax_or_vat() . '</small>';
		?>
