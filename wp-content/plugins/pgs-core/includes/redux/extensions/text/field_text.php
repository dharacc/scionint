		...
		function render() {
			...
			if ( isset( $this->field['options'] ) && ! empty( $this->field['options'] ) ) {
				...
				foreach ( $this->field['options'] as $k => $v ) {
					...
					echo '<input ' . $qtip_title . $qtip_text . 'type="text" id="' . $this->field['id'] . '-text-' . $k . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '[' . $k . ']' . '" ' . $placeholder . 'value="' . esc_attr( $v ) . '" class="regular-text ' . $this->field['class'] . '"' . $readonly . $autocomplete . ' /><br />';
					echo '</div>';
				}
				//foreach
			} else {
		...
		function enqueue() {
			if ( $this->parent->args['dev_mode'] ) {
				wp_enqueue_style(
					...
					$this->extension_url . 'field_text.css',
					array(),
					time(),
					'all'
				);
