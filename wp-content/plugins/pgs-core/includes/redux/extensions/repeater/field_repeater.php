...
// Don't duplicate me!
if ( ! class_exists( 'ReduxFramework_pgs_repeater' ) ) {
	 ...
	 * @since       1.0.0
	 */
	class ReduxFramework_pgs_repeater {
		...
		public function render() {
			...
			echo '<div class="redux-pgs_repeater-accordion" data-id="' . $this->field['id'] . '">';
			...
			if ( isset( $this->value['redux_repeater_data'] ) && is_array( $this->value['redux_repeater_data'] ) && ! empty( $this->value['redux_repeater_data'] ) ) {
				...
				foreach ( $repeaters as $repeater ) {
					...
					echo '<div class="redux-pgs_repeater-accordion-pgs_repeater" data-sortid="' . $x . '">';
					echo '<table style="margin-top: 0;" class="redux-pgs_repeater-accordion redux-pgs_repeater form-table no-border">';
					...
					echo '<h3><span class="redux-pgs_repeater-header">' . $repeater['title'] . ' </span></h3>';
					...
					if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
						echo '<a href="javascript:void(0);" class="button deletion redux-pgs_repeaters-remove">' . esc_html__( 'Delete', 'pgs-core' ) . ' ' . $this->field['item_name'] . '</a>';
					}
					echo '</div>';
					echo '</fieldset>';
					echo '</table>';
			...
			if ( 0 == $x || ( isset( $this->field['static'] ) && ( $x - 1 ) < $this->field['static'] ) ) {
				...
				while ( $loop > 0 ) {
					echo '<div class="redux-pgs_repeater-accordion-repeater">';
					echo '<table style="margin-top: 0;" class="redux-pgs_repeater-accordion redux-pgs_repeater form-table no-border">';
					echo '<fieldset class="redux-field" data-id="' . $this->field['id'] . '">';
					...
					echo '<h3><span class="redux-pgs_repeater-header"> </span></h3>';
					echo '<div>';
			...
			if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
				...
				echo '<a href="javascript:void(0);" class="button redux-pgs_repeaters-add button-primary' . $disabled . '" rel-id="' . $this->field['id'] . '-ul" rel-name="' . $this->parent->args['opt_name'] . $this->repeater_values . '[title][]">' . esc_html__( 'Add', 'pgs-core' ) . ' ' . $this->field['item_name'] . '</a><br/>';
			}
		...
		public function enqueue() {
			...
			$vars = array(
				'option_name' => $pgscore_globals['options_name'],
			);

			wp_enqueue_script(
				'redux-field-pgs-repeater-js',
			...
			wp_localize_script( 'redux-field-pgs-repeater-js', 'redux_vars', $vars );

			wp_enqueue_style(
				'redux-field-repeater-css',
				$this->extension_url . 'field_repeater' . $min . '.css',
				array(),
				time(),
				'all'
			);
		...
		function localize( $field, $value = '' ) {
			...
			if ( isset( $field['fields'] ) && ! empty( $field['fields'] ) ) {
				...
				$var = array(
					'html'     => $var . '<a href="javascript:void(0);" class="button deletion redux-pgs_repeaters-remove">' . esc_html__( 'Delete', 'pgs-core' ) . '</a>',
		...
		private function enqueue_dependencies( $field_type ) {
			...
			if ( ! class_exists( $field_class ) ) {
				$class_file = apply_filters( 'redux-typeclass-load', ReduxFramework::$_dir . 'includes/fields/' . $field_type . '/field_' . $field_type . '.php', $field_class );
