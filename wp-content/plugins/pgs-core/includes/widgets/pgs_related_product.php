...
class PGS_related_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			esc_html__( 'PGS Related Products', 'pgs-core' ), // Name.
			array( 'description' => esc_html__( 'A Related Products Sidebar Widget. This widget only work on Product details page sidebar.', 'pgs-core' ) ) // Args.
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$title          = isset( $instance['title'] ) && ! empty( $instance['title'] ) ? $instance['title'] : esc_html__( 'Related Products', 'pgs-core' );
		...
		if ( 0 === (int) count( $related ) ) {
			return;
		}

		/**
		 * Filters related products widget ids.
		 *
		 * @visible false
		 * @ignore
		 */
		...
		// Query args.
		/**
		 * Filters related products widget args.
		 *
		 * @visible false
		 * @ignore
		 */
		$query_args = apply_filters(
			'pgs_related_products_widget_args',
			array(
				'post_type'           => 'product',
				'post_status'         => 'publish',
	...
	public function form( $instance ) {
		$title       = ! empty( $instance['title'] ) ? $instance['title'] : esc_html__( 'Related Products', 'pgs-core' );
