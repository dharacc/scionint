...
if ( function_exists( 'vc_add_shortcode_param' ) ) {
	...
	vc_add_shortcode_param( 'pgscore_radio_image2', 'pgscore_radio_image_settings_field2', trailingslashit(PGSCORE_URL).'js/pgscore_radio_image2'.$suffix.'.js' );
}
...
function pgscore_radio_image_settings_field2( $settings, $value ) {
	...
	$data_show_label  = isset($settings['show_label']) && is_bool($settings['show_label'])   ? ( ($settings['show_label'] === true)  ? 'true' : 'false' ) : 'false';
	$data_hide_select = isset($settings['hide_select']) && is_bool($settings['hide_select']) ? ( ($settings['hide_select'] === true) ? 'true' : 'false' ) : 'true';
