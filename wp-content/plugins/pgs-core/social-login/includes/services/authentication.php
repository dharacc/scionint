...
function pgssl_process_login() {
	...
	if( 'login' == $auth_mode && is_user_logged_in() ) {
		...
		return pgssl_process_login_render_notice_page( sprintf( _pgssl__( "You are already logged in as %s. Do you want to <a href='%s'>log out</a>?", 'pgssl-text-domain' ), $current_user->display_name, wp_logout_url( home_url() ) ) );
	}

	// halt, if mode link and user not logged in
	if( 'link' == $auth_mode && ! is_user_logged_in() ) {
		return pgssl_process_login_render_notice_page( sprintf( _pgssl__( "You have to be logged in to be able to link your existing account. Do you want to <a href='%s'>login</a>?", 'pgssl-text-domain' ), wp_login_url( home_url() ) ) );
	}

	// halt, if mode test and not admin
	if( 'test' == $auth_mode && ! current_user_can('manage_options') ) {
		return pgssl_process_login_render_notice_page( _pgssl__( 'You do not have sufficient permissions to access this page.', 'pgssl-text-domain' ) );
	}

	// Bouncer :: Allow authentication?
	if( pgssl_social_authentication_enabled() == 2 ) {
		return pgssl_process_login_render_notice_page( _pgssl__( "Authentication through social networks is currently disabled.", 'pgssl-text-domain' ) );
	}
...
function pgssl_process_login_begin() {
	...
	if( ! pgssl_process_login_check_php_session() ) {
		return pgssl_process_login_render_notice_page( sprintf( _pgssl__( 'The session identifier is missing.', 'pgssl-text-domain' ), home_url() ) );
	}
	...
	if( ! $provider ) {
		return pgssl_process_login_render_notice_page( _pgssl__( 'Bouncer says this makes no sense.', 'pgssl-text-domain' ) );
	}
	...
	if( ! get_option( 'pgssl_settings_' . $provider . '_enabled' ) ) {
		return pgssl_process_login_render_notice_page( _pgssl__( "Unknown or disabled provider.", 'pgssl-text-domain' ) );
	}
...
function pgssl_process_login_end() {
	...
	if( 'link' == $auth_mode ) {
		...
		if( $user_id && $user_id != get_current_user_id() ) {
			return pgssl_process_login_render_notice_page( sprintf( _pgssl__( "Your <b>%s ID</b> is already linked to another account on this website.", 'pgssl-text-domain'), $provider ) );
		}
		...
		if( ! $user_id ) {
			return pgssl_process_login_render_notice_page( _pgssl__( "Sorry, we couldn't link your account.", 'pgssl-text-domain' ) );
		}
	} elseif( 'login' != $auth_mode ) {
		return pgssl_process_login_render_notice_page( _pgssl__( 'Bouncer says no.', 'pgssl-text-domain' ) );
	}
	...
	if( ! $wp_user ) {
		...
		return pgssl_process_login_render_notice_page( sprintf( _pgssl__( "Sorry, we couldn't connect you. <a href=\"%s\">Please try again</a>.", 'pgssl-text-domain' ), site_url( 'wp-login.php', 'login_post' ) ) );
	}
...
function pgssl_process_login_get_user_data( $provider, $redirect_to ) {
	...
	if( ! $user_id ) {
		...
		if( ! get_option( 'users_can_register' ) ) {
			return pgssl_process_login_render_notice_page( _pgssl__( "Registration is now closed.", 'pgssl-text-domain' ) );
		}
...
function pgssl_process_login_create_wp_user( $provider, $hybridauth_user_profile, $requested_user_login, $requested_user_email ) {
	...
	if( ! $user_id || ! is_integer( $user_id ) ) {
		...
		if ( ! is_wp_error( $user_id ) ) {
			...
			wp_mail(
				...
				wp_specialchars_decode( sprintf( __( '[%s] Your username and password info' ), $blogname ) ), // Subject
				$message,                                                                                     // Message
				''                                                                                            // Headers
			);
		}
	...
	if( ! $user_id || ! is_integer( $user_id ) ) {
		if( is_wp_error( $user_id ) ) {
			return pgssl_process_login_render_notice_page( _pgssl__( "An error occurred while creating a new user: ", 'pgssl-text-domain' ) . $user_id->get_error_message() );
		}

		return pgssl_process_login_render_notice_page( _pgssl__( "An error occurred while creating a new user!", 'pgssl-text-domain' ) );
	}
...
function pgssl_process_login_request_user_social_profile( $provider ) {
	...
	try
	{
		...
		{
			return pgssl_process_login_render_notice_page( sprintf( _pgssl__( "Sorry, we couldn't connect you with <b>%s</b>. <a href=\"%s\">Please try again</a>.", 'pgssl-text-domain' ), $provider, site_url( 'wp-login.php', 'login_post' ) ) );
		}
	}
