...
class WC_Product_Documents_Admin {
	...
	public function document_section_markup( $section, $index ) {
		...
		ob_start();
global $wpdb;
 $drol=$wpdb->get_results("select * from documentlist order by docname desc");
		?>
		<div class="wc-product-documents-section wc-metabox closed">
			...
			<h3>
				...
				<strong><?php esc_html_e( 'Name', 'woocommerce-product-documents' ); ?> &mdash;</strong> 
				
				
				<select name="product_documents_section_name[<?php echo $index; ?>]" id="product_documents_section_name_<?php echo $index; ?>" class="product_documents_section_name" />
				<?php foreach($drol as $docval){?>
				  <option value="<?php echo $docval->id;?>" <?php if(esc_attr( $section->get_name() )==$docval->id){echo "selected";}?>><?php echo $docval->docname;?></option>
				  <?php }?>
				</select>
