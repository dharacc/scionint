            ...
            $query_args['tax_query'] = array_merge($query_args['tax_query'], $this->get_tax_query(''));
			 ...
			 foreach($myresults1 as $mval)
			 {
			    $productid1[]=	$mval->ID;
		   		  ...
		   		  $query_args['post__in']   = array_merge( $productid1,$productid );
...
//                        );
    ...
    public function woof_products($atts, $is_prediction = false) {
		...
		$productidv=array();
		 if(is_user_logged_in())
		 {
			/* global $wpdb;
		         ...
		         $myresults=$wpdb->get_row("SELECT * FROM `documentuser` where userid='".$uid."'");
			 $productidv= explode(",",$myresults->productid) ;
        ...
        if (empty($product_ids)) {
			...
			$product_ids=array_merge( $product_ids,$productidv );
	      ...
	      if(is_user_logged_in())
			 ...
			 $myresults1=$wpdb->get_results("SELECT ID FROM `wp_posts` where post_type='product' and post_status='publish'  ");
			 foreach($myresults1 as $mval)
			 {
			    $productid1[]=	$mval->ID;
			 }
