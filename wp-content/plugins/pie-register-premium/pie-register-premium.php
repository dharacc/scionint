...
/* 
Plugin Name: Pie Register - Premium
...
register_activation_hook(__FILE__, 'checkPieRegisterBasicVersion');

//Action to Perform when activating
function checkPieRegisterBasicVersion()
{
	if (is_plugin_active('pie-register/pie-register.php')) {
		deactivate_plugins('pie-register/pie-register.php');
	}
}
...
if( !class_exists('PieRegister') ){
	class PieRegister extends PieReg_Base{
		...
		function pie_main(){
			...
			if( is_admin() && !is_plugin_active('pie-register/pie-register.php') && $this->piereg_pro_is_activate ) {
				
				$__api_manager 		= new Api_Manager_Example_Key();
				$_premium_keys 		= get_option("api_manager_example");
				$_arguments 		= array('email'=>$_premium_keys['activation_email'],'licence_key'=>$_premium_keys['api_key']);
				$_license_status	= $__api_manager->check($_arguments);
				$_license_status	= json_decode($_license_status);				
				
				if( $_license_status->status_check == "active" ) {
					// Look for the updates...
					require(dirname(__FILE__) . '/plugin-update-checker/plugin-update-checker.php');
					$myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
						'https://store.genetech.co/updates/pie-register-premium/plugin.json',
			...
			add_action( 'admin_notices', array( $this, 'pie_year_end_sale' ), 1 );
			add_action( 'admin_notices', array( $this, 'pie_new_year_sale_msg' ), 1 );
			...
			add_action( 'wp_ajax_dismiss_pie_year_end_sale', array( $this, 'dismiss_pie_year_end_sale' ) );	
			add_action( 'wp_ajax_dismiss_pie_new_year_sale_msg', array( $this, 'dismiss_pie_new_year_sale_msg' ) );	
			...
			if( is_plugin_active('pie-register/pie-register.php') ) {
				...
				$file_path = plugin_dir_path( __DIR__ ) . 'pie-register/pie-register.php';
				// Read the version number from the main plugin file then set it to a variable.
				$plugin_data = get_file_data( $file_path, array(
					'Version' => 'Version'
				) );
				if ($plugin_data['Version'] <= '3.6') {	
					add_action( 'admin_notices', array( $this, 'pie_warning_duplicate_plugin' ), 1 );
				}	
			}
		...
		function pieActions(){
			...
			*/
			...
			// Limit Form Submissions from a device
			add_action('wp_ajax_register_limit_submissions', array($this,'register_limit_submissions'));
			add_action('wp_ajax_nopriv_register_limit_submissions', array($this,'register_limit_submissions'));
		...
		*/
		...
		function piereg_register_scripts(){
			...
			// Register select2 script and stylesheet: https://select2.org/ // autocomplete_work
			wp_register_style( 'pie-select2css', plugins_url("/assets/css/select2.css",__FILE__), false, '3.6', 'all' );
			wp_register_script( 'pie-select2', plugins_url("/assets/js/select2.js",__FILE__), array( 'jquery' ), '3.6', true );
			...
			// For notices
			wp_enqueue_style('pie_notice_cs',plugins_url("/assets/css/pie_notice.css",__FILE__),false,'2.0', "all");
		...
		function pie_backend_enqueue_scripts($hook_s){
			...
			wp_enqueue_style('pie_font_awesome', 'https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css',false,'2.0', "all");
		}
		...
		function pie_frontend_enqueu_scripts(){
			...
			// Load select2 script and stylesheet: https://select2.org/ // autocomplete_work
			if( $this->piereg_pro_is_activate  ) {
				wp_enqueue_style( 'pie-select2css' ); 
				wp_enqueue_script( 'pie-select2' ); 
			}
			...
			if(isset($_POST['form_ids'])) {            
				...
				if(is_array($form_ids) && count($form_ids) > 0) {
					foreach($form_ids as $form_uid) {
						$form_id = (int)$form_uid;

						$time_submitted = (string) time();
						$reg_form = new Registration_form();
						$user_ip = $reg_form->get_user_ip();
				
						if ($user_ip == null)
							die(__('Unauthorised request. Access denied.','pie-register'));
				
						$limit_submission_table_name = $wpdb->prefix."pieregister_limit_submissions";
						
						// die('taha');
						
						$insert_sql = "INSERT INTO `".$limit_submission_table_name."` (`piereg_form_id`, `user_ip`) VALUES ({$form_id}, '".$user_ip."')";
						
						if(!$wpdb->query($insert_sql)){
							$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
						}else{
							$result[$form_uid] = $wpdb->insert_id;
						}
					}
				}
			}
		...
		*/
		function piereg_add_meta_box($postType) {
			...
			foreach ( $screens as $screen ) {
				add_meta_box(
					'myplugin_sectionid',
					__( 'Scion Register - '.ucwords($screen).' Restriction', 'pie-register' ),
			...
			if ( empty( $show_review_notice ) ) {				
				...
				?>
					<div class="notice notice-info pie-admin-notice-3 is-dismissible">
						<p><?php echo  sprintf(
								__( 'Hi there, we see you have been using Scion Register for a few days now, that is awesome! We hope you like it. We have a favor to ask, could you please %1$sreview us on WordPress?%2$s We like reading your feedback, and it helps us spread the word. Thank you.','pie-register'),
		...
		public function pie_license_expire_msg()
		{
			...
			$status_premium  		= get_option('piereg_api_manager_PR_Premium_status');
			$status_addon			= $this->get_addons_status();
			$renew_url 				= 'https://store.genetech.co/renew-order/';
			if ( $status_premium == "inactive" && empty( $show_review_notice ) ) {
				?>
				<!-- Your Pie Register Premium License key has expired; click here to check the status. Please renew your order to get all the plugin updates and support. -->
				...
				<?php
			} elseif($status_addon && empty( $show_review_notice )) {
				?>
					<div class="notice notice-info pie-notice-license-expire is-dismissible">
						<p><?php echo  sprintf(
								__( 'Your Pie Register Premium License key has expired; click %2$shere%3$s to check the status. Please %1$srenew%3$s your order to get all the plugin updates and support.','pie-register'),
		...
		public function pie_year_end_sale()
		{
			$show_review_notice   = get_option( 'pie_year_end_sale' );
			$now   = strtotime('today');
			$start = strtotime("12/30/2020");
			$end   = strtotime("12/31/2020");

			if ( empty( $show_review_notice ) && ($now >= $start && $now <= $end) ) {				

				$pricing_url = 'https://store.genetech.co/checkout?add-to-cart=8975';
				$img_src     = plugin_dir_url(__FILE__).'assets/images/pieregister-logo.png';
				?>
					<div class="notice notice-info pie-admin-christmas is-dismissible">
					<div class="launch-icon">
						<img src="<?php echo $img_src; ?>" alt="Pie Register Logo">
					</div>
					<div>
						<p style="font-weight:700;font-size:16px;"><?php echo ( _e( 'We are saying Farewell to 2020 with Mega Sale','pireg')); ?></p>

						<p style="font-size:15px;"><?php echo
								__( 'Get All the 11 Premium Add-ons for $14.99 ONLY until 31st December 2020.','pie-register'); ?></p>
						<p>
							<a href="<?php echo $pricing_url; ?>" target="_blank" id="pie_pricing" class="button button-primary">
								<?php _e( 'Direct Checkout', 'pie-register' ) ?>
							</a>
						</p>
					</div>
					</div>
					<script type="text/javascript">
						...
						jQuery( '.pie-admin-christmas' ).on( 'click', '.notice-dismiss', function() {		
							var data = {
								action: 'dismiss_pie_year_end_sale'
							};
							jQuery.post( ajaxurl, data );
						});
					</script>		
		...
		public function dismiss_pie_year_end_sale()
		{
			update_option( 'pie_year_end_sale', true );
		}	

		public function pie_new_year_sale_msg()
		{
			$show_review_notice   = get_option( 'pie_new_year_sale_msg' );
			$today = strtotime('today');
			$start = strtotime("01/01/2021");
			$end   = strtotime("01/10/2021");

			if ( empty( $show_review_notice ) && ($today >= $start && $today <= $end) ) {				

				$pricing_url = 'https://pieregister.com/plan-and-pricing/?utm_source=admindashboard&utm_medium=notification&utm_campaign=2021';
				$img_src     = plugin_dir_url(__FILE__).'assets/images/pieregister-logo.png';
				?>
					<div class="notice notice-info pie-admin-new-year is-dismissible">
					<div class="launch-icon">
						<img src="<?php echo $img_src; ?>" alt="Pie Register Logo">
					</div>
					<div>
						<p style="font-weight:700;font-size:16px;"><?php echo ( _e( 'New Year calls for a celebration','pireg')); ?></p>

						<p style="font-size:15px;"><?php echo
								__( 'Enjoy 40% Off on All Plans until 5th January. Use the Code <span style="font-weight:700;">2021</span> at checkout.','pie-register'); ?></p>
						<p>
							<a href="<?php echo $pricing_url; ?>" target="_blank" id="pie_pricing" class="button button-primary">
								<?php _e( 'Upgrade Your Plan Now', 'pie-register' ) ?>
							</a>
						</p>
					</div>
					</div>
					<script type="text/javascript">
						...
						jQuery( '.pie-admin-new-year' ).on( 'click', '.notice-dismiss', function() {		
							var data = {
								action: 'dismiss_pie_new_year_sale_msg'
							};
							jQuery.post( ajaxurl, data );
						});
					</script>		
		...
		public function dismiss_pie_new_year_sale_msg()
		{
			update_option( 'pie_new_year_sale_msg', true );
		}		
		...
		*/
		...
		public function pie_promo_notice_for_wc_user()
		{
			...
			if ( empty( $show_release_notice ) ) {
                ...
                <div class="notice notice-info pie-admin-notice wc_promo_users is-dismissible">
					<p><?php echo sprintf( __( ' Using WooCommerce? Synchronize the billing and shipping address on your Pie Register form with WooCommerce with our WooCommerce addon. <a target="_blank" style="color:#c30604;" id="pie_opt_in_start" href="%s">%s</a>.','pireg'),'https://pieregister.com/addons/woocommerce-addon/?utm_source=admindashboard&utm_medium=notification&utm_campaign=woocommerce',"Here's how you can do it"); ?></p> 
                </div>
                <script type="text/javascript">
		...
		public function pie_promo_notice_for_mc_user()
		{
			...
			if ( empty( $show_release_notice ) ) {
                ...
                <div class="notice notice-info pie-admin-notice mc_promo_users is-dismissible">
					<p><?php echo sprintf( __( ' MailChimp Addon is now packed with new functions which includes GDPR Agreement Field, Merge Fields and much more. Click <a target="_blank" style="color:#c30604;" id="pie_opt_in_start" href="%s">here</a> to view list of new functions.','pie-register'),'https://pieregister.com/addons/mailchimp-addon/?utm_source=admindashboard&utm_medium=notification&utm_campaign=mailchimp'); ?></p> 
                </div>
                <script type="text/javascript">					
		...
		/* 
			...
			@TODO remove this notice in the future.
		*/
		public function pie_warning_duplicate_plugin()
		{
			$show_release_notice   = get_option( 'pie_warning_duplicate_plugin' );

			if ( empty( $show_release_notice ) ) {
				
				?>
                <div class="error error-info pie-admin-notice for_pie_double is-dismissible">
					<h3><?php _e('Attention!','pie-register') ?></h3>
					<p><?php echo ( _e( 'Currently, you have both version of Pie Register is installed. Please deactivate the Pie Register - Basic version if you are using Pie Register - Premium, otherwise plugin will not work correctly.','pireg')); ?></p>
                </div>
                <?php 
            }
		}
		...
		*/
		...
		function add_pie_form_button($context)
		{
			...
			printf('<a href="%s" class="thickbox button" id="add_pie_form" title="%s"><span style="%s" class="wp-media-buttons-icon"></span>%s</a>','#TB_inline?width=480&inlineId=select_pie_form',__("Add Pie Register Form", 'pie-register'),'background: url('.plugins_url('pie-register-premium').'/assets/images/form-icon.png); background-repeat: no-repeat; background-position: left bottom;',__("Add Form","pie-register"));
...
?>
				...
				else
				{
					...
					if( $this->piereg_pro_is_activate ){
						if($this->piereg_bundle_activate) {
							...
							$errors_array = $this->pie_wp_logincheck($this->pie_post_array['log']);
							$errors = $errors_array[0];
							$error_found = $errors_array[1];
						}											
					}
		...
		function pie_wp_logincheck($user){
			global $wpdb, $errors, $wp_session;
			$errors = new WP_Error();
			$option = get_option(OPTION_PIE_REGISTER);
			$error_found = 0;

			$this->pie_post_array['log'] = $user;
			...
			if( $this->piereg_pro_is_activate ){
				...
				if( isset($option['piereg_blk_ip']) && (isset($option['enable_blockedips']) && $option['enable_blockedips'] == 1 ) ) 
				{
					...
					if( $this->isUserIpsIsBlocked( ip2long($user_ip_address), $array_ips ) ) {
						$errors->add('login-error',apply_filters("piereg_ipaddress_blocked",__('Your IP address has been blocked by the administrator.','pie-register')));
						$error_found++;
					}	
				}
				...
				if( isset($option['piereg_blk_username']) && (isset($option['enable_blockedusername']) && $option['enable_blockedusername'] == 1 ) )
				{
					...
					if( is_email($cred_userlogin) ) {
						...
						$cred_userlogin		= strtolower(is_object($userdata) ? $userdata->user_login : $cred_userlogin);
					}
					...
					if(	$this->isUserNameIsBlocked($cred_userlogin,$array_username) ) {
						$errors->add('login-error',apply_filters("piereg_username_blocked",__('This user id has been blocked by the administrator.','pie-register')));
						$error_found++;
					}	
				}
				...
				if( isset($option['piereg_blk_email']) && (isset($option['enable_blockedemail']) && $option['enable_blockedemail'] == 1 ) )
				{
					...
					if( !is_email($cred_userlogin2) ) {
						...
						$cred_userlogin2	= strtolower( is_object($userdata) ? $userdata->user_email : $cred_userlogin2);
					}
					...
					if(	$this->isEmailAddressIsBlocked($cred_userlogin2,$array_emailaddr) ) {
						$errors->add('login-error',apply_filters("piereg_email_blocked",__('This email address has been blocked by the administrator.','pie-register')));
						$error_found++;
					}	
				}
		...
		function pie_check_status_on_login($user, $password){
			...
			$block_allow_check = $this->pie_wp_logincheck($user->user_email);

			if(isset($block_allow_check[0]->errors['login-error']) && !empty($block_allow_check[0]->errors['login-error'])){
				return $block_allow_check[0];
			}
		...
		function AddPanel()
		{
			...
			$pie_page_suffix_1 = add_menu_page( "Scion Register", __('Scion Register',"pie-register") , 'manage_options', 'pie-register',  array($this,'PieRegNewForm'), plugins_url("/assets/images/pr_icon.png",__FILE__), 60 );
			...
			{
				if(!$this->piereg_pro_is_activate){
					$pie_page_suffix_14 = add_submenu_page( 'pie-register', 'Addons', '<span style="color: #82b3f1">'. __('Addons',"pie-register") .'</span>' , 'manage_options', 'pie-pro-features', array($this, 'PieRegProFeatures'));
		...
		function AddPanelOnAdminBar(){
			...
			$wp_admin_bar->add_menu( array( 
											'id' => 'pie_register',
											'title' => __( 'Pie Register', 'textdomain' ),
		...
		function pie_save_registration()
		{
			...
			if(sizeof($errors->errors) == 0)
			{
				...
				*/
				...
				if(isset($_POST['limit_submission']) && !empty($_POST['limit_submission'])){
					global $wpdb;
					$limit_submission_table_name = $wpdb->prefix."pieregister_limit_submissions";

					$time_submitted = time();
					$update_sql = "UPDATE $limit_submission_table_name SET time_submitted = '".$time_submitted."' WHERE id=".$_POST['limit_submission'];
				...
				*/
				...
				else if($option_user_verification == 3 )//Admin & E-Mail Verification
				{
					...
					$this->pie_post_array['registration_success'] = apply_filters("piereg_thank_you_for_your_registration_admin_email_veri",__("Thank you for submitting a registration request. An email will be sent when the administrator has reviewed the request.",'pie-register'));
				}
		...
		function PieRegSettingsProcess() {
			...
			if(isset($_POST['piereg_settings_allusers']) && wp_verify_nonce( $_POST['piereg_settings_allusers'], 'piereg_wp_settings_allusers' )) {
				...
				if($this->piereg_pro_is_activate){
					$update['piereg_security_attempts_login_value'] = isset($this->pie_post_array['piereg_security_attempts_login_value']) ? intval($this->pie_post_array['piereg_security_attempts_login_value']) : "0";
				}
				...
				$update['reg_form_submission_limit'] = isset($this->pie_post_array['reg_form_submission_limit']) ? intval($this->pie_post_array['reg_form_submission_limit']):0;
				...
				if(isset($_POST['piereg_redirect_settings_nonce']) && wp_verify_nonce( $_POST['piereg_redirect_settings_nonce'], 'piereg_wp_redirect_settings_nonce' ))
				{
					...
					elseif(
						...
						if($wpdb->query( $wpdb->prepare($sql, $insr_piereg_user_role, $insr_logged_in_url, $insr_log_in_page, $insr_log_out_url, $insr_log_out_page, $insr_status) ))
							$this->pie_post_array['notice'] = __("Record added successfully.","pie-register");
						else
							$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
					}else{
			...
			elseif(isset($this->pie_post_array['redirect_settings_update'])){
				...
				if(isset($this->pie_post_array['id']) && $this->pie_post_array['id'] && isset($this->pie_post_array['logged_in_url']) && isset($this->pie_post_array['log_out_url'])){
					...
					if( $wpdb->query( $wpdb->prepare($sql, $upd_logged_in_url, $upd_log_in_page, $upd_log_out_url, $upd_log_out_page, $upd_id) ) )
						$this->pie_post_array['notice'] = __("Record updated successfully.","pie-register");
					else
						$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
				}else{
					$this->pie_post_array['error'] = __("Invalid field(s)","pie-register");
		...
		function PieRegInvitationCodes()
		{
			...
			if( isset($_POST['piereg_invitation_code_nonce']) && wp_verify_nonce( $_POST['piereg_invitation_code_nonce'], 'piereg_wp_invitation_code_nonce' ) )
			{
				...
				if( isset($this->pie_post_array['submit_invite_email']) &&  $this->piereg_pro_is_activate ){
					...
					$piereg['pie_email_subject'] 	= isset($this->pie_post_array['pie_email_subject']) ? $this->disable_magic_quotes_gpc($this->pie_post_array['pie_email_subject']) : '';
					$piereg['pie_email_content'] 	= isset($this->pie_post_array['pie_email_content']) ? $this->disable_magic_quotes_gpc($this->pie_post_array['pie_email_content']) : '';
					$_data_updated	= true;
				}
							...
							if(!empty($from_email) && filter_var($from_email,FILTER_VALIDATE_EMAIL)){ 
								...
								foreach($to_emails as $email){
									...
									if( get_user_by('email', $email) ){
										$this->pie_post_array['error'][] = sprintf(__("User %s already exists.","pie-register"),$email);
										continue;
									}
									...
									if(wp_mail($email, $subject, $this->nl2br_save_html($emailBody), $headers)){
						...
						if(isset($this->pie_post_array['add_code'])){
							...
							if($this->piereg_pro_is_activate){
								if(empty($this->pie_post_array['invitation_code_prefix']) && !empty($this->pie_post_array['invitation_code_numbers'])){
									$this->pie_post_array['error'] = __("Invitation code prefix is empty.","pie-register");
								}
								if(empty($this->pie_post_array['invitation_code_prefix']) && empty($this->pie_post_array['invitation_code_numbers'])){
							...
							if(isset($this->pie_post_array['invitation_code_usage']) && !is_numeric($this->pie_post_array['invitation_code_usage']) && trim($this->pie_post_array['invitation_code_usage']) != ''){
								...
								$this->pie_post_array['error'] .= __("Must be a number greater than zero.","pie-register");
		...
		*/
		...
		function addDate($field,$no)
		{
			...
			echo	'<div class="time date_format_field" id="datepicker_'.$no.'" '.$datepicker.'>
					  <input disabled="disabled" type="text" class="input_fields">
	  				  <img src="'.plugins_url('pie-register-premium').'/assets/images/calendar.png" id="calendar_image_'.$no.'" '.$calendar_icon.' /> </div>';
		...
		{
			echo '<img src="'.plugins_url('pie-register-premium').'/assets/images/pagebreak.png" style="max-width:100%;" />';
		}
		...
		function addCaptcha($field,$no)
		{
			...
			if(isset($field['recaptcha_type']) && $field['recaptcha_type'] == 2)
				echo '<img id="captcha_img" src="'.plugins_url('pie-register-premium').'/assets/images/new-recatpcha.png" data-captcha-img-src="'.plugins_url('pie-register-premium').'/assets/images/" />';
			else
				echo '<img id="captcha_img" src="'.plugins_url('pie-register-premium').'/assets/images/recatpcha.jpg" data-captcha-img-src="'.plugins_url('pie-register-premium').'/assets/images/" />';
		...
		function addMath_Captcha($field,$no)
		{
			...
			echo '<img id="captcha_img" src="'.plugins_url('pie-register-premium').'/assets/images/math_catpcha.png" />';	
			echo '<input type="hidden" id="default_'.$field['type'].'">';
		}
		function addList($field,$no)
		{
			...
			echo '<img src="'.plugins_url('pie-register-premium').'/assets/images/plus.png" />';
		}
		...
		function get_paypalstandard($value,	$payment_gateways=array())
		{
			...
			{
				$data .= '<img src="'.plugins_url('pie-register-premium/assets/images/paypal_std_btn.png').'">';
		...
		function Add_payment_option($send_data,$paymeny_gatways) // Only For Paypal
		{
			...
			if($check_payment["enable_paypal"] == 1 && !(empty($check_payment['paypal_butt_id'])) )
			{
				...
				{
					$send_data .= '<option data-img="'.plugins_url('pie-register-premium/assets/images/paypal_std_btn.png').'" value="PaypalStandard">'.(__("Paypal (Standard)","pie-register")).'</option>';
				}
			}
						...
						case "pie_us_states":
							if($arr_val){
								...
								if(is_array($result_merge_array)){
									...
									$this->pie_post_array['success_message'] = __("Settings imported successfully","pie-register");
								}
								unset($db_option);
		...
		function import_invitation_codes($_is_csv=false){
			if(isset($_POST['piereg_import_invitations_codes']) && wp_verify_nonce( $_POST['piereg_import_invitations_codes'], 'piereg_wp_import_invitations_codes' ))
			{
				...
				if(isset($array_json_file['invitation_codes']) && is_array($array_json_file['invitation_codes'])){
					...
					$this->pie_post_array['success_message'] = __("Invitation codes imported successfully","pie-register");					
				...
				if(isset($array_json_file['admin_email_template'],$array_json_file['user_email_template'])){
					if(is_array($array_json_file['admin_email_template']) && is_array($array_json_file['user_email_template'])){
						...
						if(is_array($merge_email_templates)){
							...
							$this->pie_post_array['success_message'] = __("Email templates imported successfully","pie-register");
						}
					}
				}
			}else{
					...
					foreach( $users as $user ) 
					{
						...
						if ( !empty($uploads) )
						{
							foreach( $uploads as $upload )
							{
								...
								{
									$files[$upload->user_id.'_'.$count] = $upload->meta_value;
								}
								$count++;
							}
						}
