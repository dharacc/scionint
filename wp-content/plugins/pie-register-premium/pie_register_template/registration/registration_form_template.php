...
class Registration_form_template extends Registration_form
{
	...
	function addUpload()
	{
		...
		$data  = '<div class="fieldset">';
		$data .= $this->addLabel();
		$data .= '<input '.$this->read_only.' id="'.$this->id.'" name="'.$this->name.'" data-field_id="'.$this->get_pr_widget_prefix().'piereg_field_'.$this->no.'" class="'.$this->addClass().' '.$cl_class.'"  '.$this->addValidation().' '.$cl_data.' type="file"  />';
	...
	function addProfilePicUpload()
	{
		...
		$data  = '<div class="fieldset">';
		$data .= $this->addLabel();
		$data .= '<input '.$this->read_only.' id="'.$this->id.'" data-field_id="'.$this->get_pr_widget_prefix().'piereg_field_'.$this->no.'" name="'.$this->name.'" class="'.$this->addClass().' piereg_validate[funcCall[checkExtensions],ext[gif|GIF|jpeg|JPEG|jpg|JPG|png|PNG|bmp|BMP]] '.$cl_class.'"  '.$this->addValidation().' '.$cl_data.' type="file"  />';
		$data .= $this->addDesc();
		$data .= '</div>';
		return $data;
	}
	...
	function addTime()
	{
		...
		$data .= '<input '.$this->read_only.' value="'.( (isset($time_this_values["hh"])) ? $time_this_values["hh"] : "" ).'" maxlength="2" id="hh_'.$this->id.'" name="'.$this->name.'[hh]" type="text"  class="'.$this->addClass().' '.$cl_class.'" '.$cl_data.' '.$this->addValidation().'>';
	...
	{ 
		...
		$_customize_search = ""; // autocomplete_work
		if( $this->piereg_pro_is_activate && isset($this->field['is_search']) ){
			$_customize_search = " display_search";			
		}
		...
		$data .= '<select '.$this->read_only.' '.$multiple.' id="'.$field_id.'" name="'.$name.'" data-field_id="'.$this->get_pr_widget_prefix().'piereg_field_'.$this->no.'" class="'.$this->addClass().' '.$cl_class.$_customize_search.'"'.$cl_data.' '.$this->addValidation().'  >'; // autocomplete_work
		...
		$data = apply_filters('piereg_select_field', $data, $this->field, 'registration');
	...
	function addPhone()
	{
		...
		$data .= '<input '.$this->read_only.' id="'.$this->id.'" class="'.$this->addClass().' '.$cl_class.'"'.$cl_data.' data-field_id="'.$this->get_pr_widget_prefix().'piereg_field_'.$this->no.'"  '.$this->addValidation().' placeholder="'.((isset($this->field['placeholder']))?$this->field['placeholder']:"").'" name="'.$this->name.'" type="text" value="'.$this->getDefaultValue().'" />';
		$data .= $this->addDesc();
		$data .= '</div>';
		return $data;
	}
	function addList()
	{
		...
		for($a = 1 ,$c=0; $a <= $this->field['rows'] ; $a++,$c++)
		{
			if($a==1)
			{
				...
				{
					$data .= ' <img src="'.plugins_url('pie-register-premium').'/assets/images/plus.png" onclick="addList('.$this->field['rows'].','.$this->field['id'].');" alt="add" /></div>';		
				}
			...
			else
			{
				...
				for($b = 1 ; $b <= $this->field['cols'] ;$b++)
				{
					...
					$data .= ' <img src="'.plugins_url('pie-register-premium').'/assets/images/minus.gif" onclick="removeList('.$this->field['rows'].','.$this->field['id'].','.$a.');" alt="add" />';
					$data .= '</div>';
			}
	...
	function addCheckRadio()
	{
		...
		if(sizeof($this->field['value']) > 0)
		{
			...
			for($a = 0 ; $a < sizeof($this->field['value']) ; $a++)
			{
				...
				$data .= '<input '.$id.' '.$this->read_only.' '.$checked.' value="'.$this->field['value'][$a].'" data-field_id="'.$this->get_pr_widget_prefix().'piereg_field_'.$this->no.'" type="'.$this->field['type'].'" name="'.$this->name.'[]" class="'.$this->addClass("input_fields").' radio_fields '.$cl_class.'" '.$cl_data.' '.$this->addValidation().' data-map-field-by-class="'.$dymanic_class.'" >';
				$data .= "<label>";
					$data .= $this->field['display'][$a];
				$data .= "</label>";
				$data .= '</div>';
	...
	function addAddress()
	{
		...
		$data .= '<input '.$this->read_only.' type="text" name="'.$this->name.'[address]" id="'.$this->id.'" class="'.$this->addClass().' '.$cl_class.'"  '.$this->addValidation().' '.$cl_data.' value="'.((isset($address_values['address']))?$address_values['address']:"").'">';
	...
	function addDate()
	{
		...
		if($this->field['date_type'] == "datefield")
		{
			...
			if($this->field['date_format']=="mm/dd/yy")
			{
				$data .= '<div class="piereg_time date_format_field">';
                    $data .= '<div class="time_fields">';
                        $data .= '<input '.$this->read_only.' id="mm_'.$this->id.'" name="'.$this->name.'[date][mm]" maxlength="2" type="text" class="'.$this->addClass("input_fields",array("custom[month]")).' '.$cl_class.'" '.$cl_data.' '.$this->addValidation().' value="'.((isset($date_this_values['date']['mm']))?$date_this_values['date']['mm']:"").'">';
                        $data .= '<label>'.__("MM","pie-register").'</label>';
                    $data .= '</div>';
                    $data .= '<div class="time_fields">';
			...
			else if($this->field['date_format']=="yy/mm/dd" || $this->field['date_format']=="yy.mm.dd")
			{
				$data .= '<div class="piereg_time date_format_field">';
                    $data .= '<div class="time_fields">';
                        $data .= '<input '.$this->read_only.' id="yy_'.$this->id.'" name="'.$this->name.'[date][yy]" maxlength="4"  type="text" class="'.$this->addClass("input_fields",array("custom[year]")).' '.$cl_class.'" '.$cl_data.' value="'.((isset($date_this_values['date']['yy']))?$date_this_values['date']['yy']:"").'">';
                        $data .= '<label>'.__("YYYY","pie-register").'</label>';
                    $data .= '</div>';
                    $data .= '<div class="time_fields">';
			...
			else if($this->field['date_format']=="dd/mm/yy" || $this->field['date_format']=="dd-mm-yy" || $this->field['date_format']=="dd.mm.yy")
			{
                $data .= '<div class="piereg_time date_format_field">';
                    $data .= '<div class="time_fields">';
                        $data .= '<input '.$this->read_only.' id="dd_'.$this->id.'" name="'.$this->name.'[date][dd]" maxlength="2"  type="text" class="'.$this->addClass("input_fields",array("custom[day]")).' '.$cl_class.'" '.$cl_data.' value="'.((isset($date_this_values['date']['dd']))?$date_this_values['date']['dd']:"").'">';
                        $data .= '<label>'.__("DD","pie-register").'</label>';
                    $data .= '</div>';
                    $data .= '<div class="time_fields">';
                        $data .= '<input '.$this->read_only.' id="mm_'.$this->id.'" name="'.$this->name.'[date][mm]" maxlength="2" type="text" class="'.$this->addClass("input_fields",array("custom[month]")).'" '.$this->addValidation().' value="'.((isset($date_this_values['date']['mm']))?$date_this_values['date']['mm']:"").'">';
		...
		else if($this->field['date_type'] == "datepicker")
		{
				if( $this->field['calendar_icon'] == "calendar" || $this->field['calendar_icon'] == "custom" ) 
				  ...
				  $data .= '<input '.$this->read_only.' id="'.$this->id.'" name="'.$this->name.'[date][]" type="text" class="'.$this->addClass().' date_start '.$cl_class.'" '.$cl_data.' title="'.$this->field['date_format'].'" value="';
				...
				{
					 $data .=  '<img id="'.$this->id.'_icon" class="calendar_icon" src="'.plugins_url('pie-register-premium').'/assets/images/calendar.png" />';
				}
		...
		else if($this->field['date_type'] == "datedropdown")
		{
			...
			if($this->field['date_format']=="mm/dd/yy")
			{
					...
					$data .= '<div class="piereg_time date_format_field">';
					  $data .= '<div class="time_fields">';
						$data .= '<select '.$this->read_only.' id="mm_'.$this->id.'" name="'.$this->name.'[date][mm]" class="'.$this->addClass("input_fields",array("custom[month]")).' '.$cl_class.'" '.$$cl_data.' '.$this->addValidation().'>';
			...
			else if($this->field['date_format']=="yy/mm/dd" || $this->field['date_format']=="yy.mm.dd")
			{
					$data .= '<div class="piereg_time date_format_field">';
					 $data .= '<div class="time_fields">';
					$data .= '<select '.$this->read_only.' id="yy_'.$this->id.'" name="'.$this->name.'[date][yy]" class="'.$this->addClass("input_fields",array("custom[year]")).' '.$cl_class.'" '.$cl_data.'>';
					  $data .= '<option value="">'.__("Year","pie-register").'</option>';
					  for($a=((int)date("Y"));$a>=(((int)date("Y"))-100);$a--){
						  if(isset($date_this_values['date']['yy']) && $date_this_values['date']['yy'] == $a)
						  	$sel = ' selected=""';
			...
			else
			{
				$data .= '<div class="piereg_time date_format_field">';
				  $data .= '<div class="time_fields">';
					$data .= '<select '.$this->read_only.' id="dd_'.$this->id.'" name="'.$this->name.'[date][dd]" class="'.$this->addClass("input_fields",array("custom[day]")).' '.$cl_class.'" '.$cl_data.'>';
					  $data .= '<option value="">'.__("Day","pie-register").'</option>';
					  for($a=1;$a<=31;$a++){
						  if(isset($date_this_values['date']['dd']) && $date_this_values['date']['dd'] == $a)
						  	$sel = ' selected=""';
	...
	function addInvitationField(){
		...
		$data .= $this->addLabel();
		$data .= '<input id="'.$this->id.'" '.$this->addValidation().' name="invitation" class="'.$this->addClass().' '.$cl_class.'"'.$cl_data.' data-field_id="'.$this->get_pr_widget_prefix().'piereg_field_'.$this->no.'" placeholder="'.$this->field['placeholder'].'" type="'.$type.'" value="'.$invite_code.'" />';
		$data .= $this->addDesc();
		$data .= '</div>';
		return $data;
	}
		...
		if(is_array($this->data)){
			...
			foreach($this->data as $this->field)
			{
				...
				if($this->field['type'] == "form"){
					...
					$pie_reg_fields .= '<input type="hidden" value="" name="limit_submission" />';
					continue;
				}
