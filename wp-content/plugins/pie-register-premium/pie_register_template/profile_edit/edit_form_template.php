...
class Edit_form_template extends Edit_form
{
	...
	function addUsername(){
		...
		// '.$cl_class.'"'.$cl_data.'
		$data  = '<div class="fieldset">'.$this->addLabel();
		$data .= '<input id="username" type="text" data-field_id="piereg_field_'.$this->no.'" value="'.$this->user->data->user_login.'" readonly="readonly" disabled="disabled" class="'.$this->field['css'].' input_fields '.$cl_class.'"'.$cl_data.' />';
		$data .= $this->addDesc();
		//$data .= $this->made_conditional_logic();
		$data .= '</div>';
		return $data;
	...
	function addUpload()
	{
		...
		$data .= '<input '.$this->read_only.' id="'.$this->id.'" name="'.$this->name.'" data-field_id="piereg_field_'.$this->no.'" class="'.$this->addClass().' '.$cl_class.'" '.$cl_data.' '.$this->addValidation().' type="file"  />';
	...
	function addProfilePic()
	{
		...
		$data .= '<input '.$this->read_only.' id="'.$this->id.'" name="'.$this->name.'" data-field_id="piereg_field_'.$this->no.'" class="'.$this->addClass().' piereg_validate[funcCall[checkExtensions],ext[gif|GIF|jpeg|JPEG|jpg|JPG|png|PNG|bmp|BMP]] re-upload-pic '.$cl_class.'" '.$cl_data.' '.$this->addValidation().' type="file"  />';
		$data .= '<input id="'.$this->id.'_hidden" name="'.$this->name.'_hidden" value="'.$val.'" type="hidden"  />';
	...
	function addTime(){
		...
		$data .= '<div class="piereg_time">
					<div class="time_fields">
						<input '.$this->read_only.' maxlength="2" id="hh_'.$this->id.'" name="'.$this->name.'[hh]" type="text"  class="'.$this->addClass().' '.$cl_class.'" '.$cl_data.' '.$this->addValidation().' value="'.((isset($val['hh']))?$val['hh'] : "").'">
	...
	function addDropdown(){
		...
		$data .= '<select '.$this->read_only.' '.$multiple.' id="'.$this->name.'" data-field_id="piereg_field_'.$this->no.'" name="'.$name.'" class="'.$this->addClass().' '.$cl_class.$_customize_search.'"'.$cl_data.' '.$this->addValidation().' >'; // autocomplete_work
		...
		$data = apply_filters('piereg_select_field', $data, $this->field, 'profile');
		...
		if(sizeof($this->field['value']) > 0)
		{
			...
			for($a = 0 ; $a < sizeof($this->field['value']) ; $a++)
			{
				...
				if(!empty($this->field['display'][$a]))
				{
					...
					$data .= "<label>";
					$data .= '<input '.$id.' '.$this->read_only.' '.$checked.' value="'.$this->field['value'][$a].'" data-field_id="piereg_field_'.$this->no.'" type="'.$this->field['type'].'" name="'.$this->name.'[]" class="'.$this->addClass("").' radio_fields '.$cl_class.'" '.$cl_data.' '.$this->addValidation().' data-map-field-by-class="'.$dymanic_class.'" >';
	...
	function addAddress()
	{
		...
		$data .= '<div class="address">
		  <input '.$this->read_only.' type="text" name="'.$this->name.'[address]" id="'.$this->id.'" class="'.$this->addClass().' '.$cl_class.'" '.$cl_data.' '.$this->addValidation().' value="'.((isset($val['address']))?$val['address']:"").'">
	...
	function addDate()
	{
		...
		if($this->field['date_type'] == "datefield")
		{
			...
			$data .= '<div class="piereg_time date_format_field">
				  <div class="time_fields">
					<input '.$this->read_only.' id="mm_'.$this->id.'" name="'.$this->name.'[date][mm]" maxlength="2" type="text" class="'.$this->addClass("input_fields",array("custom[month]")).' '.$cl_class.'"'.$cl_data.' '.$this->addValidation().' value="'.((isset($val['date']['mm']))?$val['date']['mm']:"").'">
					<label>'.__("MM","pie-register").'</label>
				  </div>
				  <div class="time_fields">
			...
			else if($this->field['date_format']=="yy/mm/dd" || $this->field['date_format']=="yy.mm.dd")
			{
				$data .= '<div class="piereg_time time date_format_field">
				 <div class="time_fields">
					<input '.$this->read_only.' value="'.(isset($val['date']['yy'])?$val['date']['yy']:"").'" id="yy_'.$this->id.'" name="'.$this->name.'[date][yy]" maxlength="4"  type="text" class="'.$this->addClass("input_fields",array("custom[year]")).' '.$cl_class.'" '.$cl_data.'>
					<label>'.__("YYYY","pie-register").'</label>
				  </div>
				  <div class="time_fields">
			...
			else
			{
				$data .= '<div class="piereg_time date_format_field">
				 <div class="time_fields">
					<input '.$this->read_only.' value="'.(isset($val['date']['dd']) ? $val['date']['dd'] :"").'" id="dd_'.$this->id.'" name="'.$this->name.'[date][dd]" maxlength="2" type="text" class="'.$this->addClass("input_fields",array("custom[day]")).' '.$cl_class.'"'.$cl_data.'>
					<label>'.__("DD","pie-register").'</label>
				  </div>				 
				  <div class="time_fields">
		...
		else if($this->field['date_type'] == "datepicker")
		{
			...
			else
			{
				...
				$data .= '<input '.$this->read_only.' id="'.$this->id.'" name="'.$this->name.'[date][]" readonly="readonly" type="text" class="'.$this->addClass().' date_start '.$cl_class.'" '.$cl_data.' value="'.$val.'">';
		...
		else if($this->field['date_type'] == "datedropdown")
		{
			if($this->field['date_format']=="mm/dd/yy")
			{
					$data .= '<div class="piereg_time date_format_field">
				  <div class="time_fields">
					<select '.$this->read_only.' id="mm_'.$this->id.'" name="'.$this->name.'[date][mm]" class="'.$this->addClass("").' '.$cl_class.'" '.$cl_data.' '.$this->addValidation().'>
			...
			else if($this->field['date_format']=="yy/mm/dd" || $this->field['date_format']=="yy.mm.dd")
			{
					$data .= '<div class="piereg_time date_format_field">
					 <div class="time_fields">
					<select '.$this->read_only.' id="yy_'.$this->id.'" name="'.$this->name.'[date][yy]" class="'.$this->addClass("").' '.$cl_class.'" '.$cl_data.'>
					  <option value="">'.__("Year","pie-register").'</option>';
					  for($a=((int)date("Y"));$a>=(((int)date("Y"))-100);$a--){
					  	$data .= '<option value="'.$a.'"';
			...
			else
			{
				$data .= '<div class="piereg_time date_format_field">';
				  ...
				  $data .=
				  '<div class="time_fields">
					<select '.$this->read_only.' id="dd_'.$this->id.'" name="'.$this->name.'[date][dd]" class="'.$this->addClass("").' '.$cl_class.'" '.$cl_data.'>
					  <option value="">'.__("Day","pie-register").'</option>';
					  for($a=1;$a<=31;$a++){
					  	$data .= '<option value="'.str_pad($a, 2, "0", STR_PAD_LEFT).'"';
						$data .= (isset($val['date']) && $val['date']['dd'] == $a)? 'selected="selected"' : "";
