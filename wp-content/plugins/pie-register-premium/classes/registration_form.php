...
class Registration_form extends PieReg_Base
{
		...
		if( $this->piereg_pro_is_activate )	
		{
			if($this->piereg_bundle_activate) {
				...
				if( isset($piereg['piereg_blk_ip']) && (isset($piereg['enable_blockedips']) && $piereg['enable_blockedips'] == 1) )
				{
					...
					{
						$errors->add( "blk_ipaddress" , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.apply_filters("piereg_ipaddress_blocked",__('Your IP address has been blocked by the administrator.','pie-register' )));
					}
				}
				...
				if( isset($piereg['piereg_blk_username']) && (isset($piereg['enable_blockedusername']) && $piereg['enable_blockedusername'] == 1) )
				{
					...
					{
						$errors->add( "blk_username" , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.apply_filters("piereg_username_blocked",__('This user id has been blocked by the administrator.','pie-register' )));
					}
				}
				...
				if( isset($piereg['piereg_blk_email']) && (isset($piereg['enable_blockedemail']) && $piereg['enable_blockedemail'] == 1) )
				{
					...
					if(	$this->isEmailAddressIsBlocked(strtolower($this->pie_post_array['e_mail']),$array_emailaddr) ) {
						$errors->add( "blk_email" , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.apply_filters("piereg_email_blocked",__('This email address has been blocked by the administrator.','pie-register' )));					
			...
			// Limiting number of submissions for a device
			if ($this->limit_submission_for_device($this->form_id, $piereg['reg_form_submission_limit'])){
				$errors->add( "piereg_form_limit_submissions" , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.apply_filters("piereg_form_limit_submissions",__('To fight spam admin has fixed the maximum number of submissions for this form from a single device. You can resubmit after 24 hours or you can contact the admin to reset the limit.','pie-register' )));
			}
		}
		/*
		...
		if( isset($piereg['reg_form_submission_time_enable']) && $piereg['reg_form_submission_time_enable'] == 1 )
		{
			if( isset($this->pie_post_array['prereg_form_submission']) ){
				...
				if( empty($this->pie_post_array['prereg_form_submission']) ){
					$errors->add( "piereg_form_time_submission" , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.apply_filters("piereg_form_time_submission",__('Spamming is not allowed.','pie-register' )));
				}else{
				...
				$errors->add( "form_time_sibmission" , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.apply_filters("piereg_form_time_sibmission",__('Spamming is not allowed.','pie-register' )));
			}
		}
		if(is_array($this->data)){
			...
			*/
			...
			if( $validation_message == "" && $required != "" )
			{
				if( $field['type'] !== 'form' )
				{
					if($field['type'] == 'date' || $field['type'] == 'time' || $field['type'] == 'address'){
						foreach($this->pie_post_array[$slug] as $val_field){
							if(is_array($val_field)){
								foreach($val_field as $val_data){
									if($val_data == ''){
										$empty_field = false;
										break;
									}
	...
	function limit_submission_for_device($form_id,$submission_limit){
		global $wpdb;
        ...
        $start_ts = $now - $t;
        $end_ts   = $start_ts + $in_a_day - 1;

        $ip = $this->get_user_ip();
		$limit_submission_table_name = $wpdb->prefix."pieregister_limit_submissions";
		$res = "SELECT COUNT(`id`) as count FROM {$limit_submission_table_name} WHERE  `piereg_form_id` = {$form_id} AND `user_ip` = '{$ip}' AND `time_submitted` BETWEEN '{$start_ts}' AND '$end_ts'";
		$results = $wpdb->get_results($res);
		...
		if ((int) $results[0]->count < $submission_limit)
            return false;
        else
            return true;
	}
