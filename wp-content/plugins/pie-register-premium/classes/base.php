	...
	class PieReg_Base extends PieRegisterBaseVariables
	{
		...
		*/
		...
		function install_settings()
		{
			...
			$update['reg_form_submission_limit'] 		= (isset($current["reg_form_submission_limit"]) && $current['reg_form_submission_limit'])? $current['reg_form_submission_limit'] : "0";
			...
			*/
			...
			$limit_submission_table_sql  = "CREATE TABLE IF NOT EXISTS $limit_submission_table_name (
											`id` int NOT NULL PRIMARY KEY AUTO_INCREMENT,
											`piereg_form_id` int NOT NULL,
											`user_ip` varchar(50) NOT NULL,
											`time_submitted` varchar(50)
											) ENGINE=MYISAM;";
			
			if(!$wpdb->query($limit_submission_table_sql))
			{
				$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
			}
		...
		function getDefaultMeta()
		{
			...
			if( $this->piereg_pro_is_activate )
			{
				$structure["dropdown"] .= '<div class="advance_fields"><label>'.__("Search Filter","pie-register").'</label><input name="field[%d%][is_search]" id="is_search_%d%" value="%d%" type="checkbox" class="checkbox_fields"></div><div class="advance_fields"><label>'.__("Enable Conditional Logic","pie-register").'</label><select name="field[%d%][conditional_logic]" data-conditional_area="conditional_area_%d%" id="conditional_logic_%d%"  class="checkbox_fields enabel_conditional_logic"><option value="1">'.__("Yes","pie-register").'</option><option value="0" selected="selected" >'.__("No","pie-register").'</option></select></div><div class="advance_fields" id="conditional_area_%d%" style="display:none;"><label for="field_status_%d%"></label><select name="field[%d%][field_status]" id="field_status_%d%"  class="field_status" style="width:auto;"><option value="1" selected="selected">'.__("Show","pie-register").'</option><option value="0">'.__("Hide","pie-register").'</option></select><span style="color:#fff;"> '.__("this field if","pie-register").' </span><input type="hidden" name="field[%d%][selected_field]" id="selected_field_%d%" class="selected_field_input"><select data-selected_field="selected_field_%d%" class="selected_field piereg_all_field_dropdown" style="width:100px;"></select><select class="field_rule_operator_select" id="field_rule_operator" name="field[%d%][field_rule_operator]" style="width:auto;"><option selected="selected" value="==">equal</option><option value="!=">not equal</option><option value="empty">empty</option><option value="not_empty">not empty</option><option value=">">greater than</option><option value="<">less than</option><option value="contains">contains</option><option value="starts_with">starts with</option><option value="ends_with">ends with</option><option value="range">range</option></select><div class="wrap_cond_value"><input type="text" name="field[%d%][conditional_value]" id="conditional_value_%d%" class="input_fields conditional_value_input" placeholder="Enter Value"></div></div>'; // autocomplete_work
			}
			...
			if( $this->piereg_pro_is_activate )
			{
				$structure["multiselect"] .= '<div class="advance_fields"><label>'.__("Search Filter","pie-register").'</label><input name="field[%d%][is_search]" id="is_search_%d%" value="%d%" type="checkbox" class="checkbox_fields"></div>'; 
			}
...
// pie-register-woocommerce Addon
		...
		$structure["address"] .= '</div></div>';	
			...
			$structure['invitation'] = '<div class="fields_main"><div class="advance_options_fields"><div class="advance_fields"><label for="label_%d%">'.__("Label","pie-register").'</label><input type="text" name="field[%d%][label]" id="label_%d%" class="input_fields field_label"></div><div class="advance_fields"><label for="desc_%d%">'.__("Description","pie-register").'</label><textarea name="field[%d%][desc]" id="desc_%d%" rows="8" cols="16"></textarea></div><div class="advance_fields"><label for="required_%d%">'.__("Rules","pie-register").'</label><input name="field[%d%][required]" id="required_%d%" value="%d%" type="checkbox" class="checkbox_fields"><label for="required_%d%" class="required">'.__("Required","pie-register").'</label></div><div class="advance_fields"><label for="placeholder_%d%">'.__("Placeholder","pie-register").'</label><input type="text" name="field[%d%][placeholder]" id="placeholder_%d%" class="input_fields field_placeholder"></div><div class="advance_fields"><label for="validation_message_%d%">'.__("Validation Message","pie-register").'</label><input type="text" name="field[%d%][validation_message]" id="validation_message_%d%" class="input_fields"></div><div class="advance_fields"><label for="css_%d%">'.__("CSS Class Name","pie-register").'</label><input type="text" name="field[%d%][css]" id="css_%d%" class="input_fields"></div>';
				...
				foreach($result_invitaion_code as $object){
					if( ( $object->expiry_date == "0000-00-00" || strtotime($object->expiry_date) > strtotime($today) ) && intval($object->status) != 0 && intval($object->code_usage) > 0 && intval($object->code_usage) > intval($object->count) ){
		...
		*/
		...
		public function pie_premium_check()
		{
			...
			$response 	= false;
			$_pie_premium_addon_user = get_option('pie_premium_version');			
			$user_email = (isset($options['activation_email']) && !empty($options['activation_email'])) ? $options['activation_email'] : get_option('admin_email');

			
			if(!$_pie_premium_addon_user) {
				$platform 	= site_url();
				$target_url = add_query_arg( ['wc-api'=>'am-software-api','request'=>'add_premium_version_user','platform'=>urlencode($platform),'activation_email'=>$user_email], $this->upgrade_url );
				...
				if(isset($response['logged_in'])) {
					update_option('pie_premium_version','yes');
				}
			}
		...
		function piereg_default_settings()
		{
			...
			$update['reg_form_submission_limit'] = "0";
				...
				case 'piereg_settings_security_advanced';
					foreach ($post as $key => $value)
					{
						switch ($key)
						{
							...
							case 'reg_form_submission_limit':
								$post[$key] = $this->sanitize_validate_as('number',$value);
								break;
		...
		*/		
		...
		function checkIfConditionApplied( $field, $metakey=false, $fields_reg_form=false){
			...
			if( $fields_reg_form )
			{
				...
				if(is_array($_POST[$slug_matched])){
					$_POST[$slug_matched] = $_POST[$slug_matched][0];
				}
