	...
	protected function set_url( $connected_account, $endpoint_slug, $params ) {
		...
		if ( $account_type === 'basic' ) {
			...
			if ( $endpoint_slug === 'header' ) {
				...
				$url = 'https://graph.facebook.com/'.$connected_account['user_id'].'/stories?fields=media_url,caption,id,media_type,permalink,children%7Bmedia_url,id,media_type,permalink%7D&limit=100&access_token='.sbi_maybe_clean( $connected_account['access_token'] );
			} elseif ( $endpoint_slug === 'hashtags_top' ) {
				$num = min( $num, 50 );
				$url = 'https://graph.facebook.com/v7.0/'.$params['hashtag_id'].'/top_media?user_id='.$connected_account['user_id'].'&fields=media_url,caption,id,media_type,timestamp,comments_count,like_count,permalink,children%7Bmedia_url,id,media_type,permalink%7D&limit='.$num.'&access_token='.sbi_maybe_clean( $connected_account['access_token'] );
			} elseif ( $endpoint_slug === 'hashtags_recent' ) {
				$num = min( $num, 50 );
				$url = 'https://graph.facebook.com/v7.0/'.$params['hashtag_id'].'/recent_media?user_id='.$connected_account['user_id'].'&fields=media_url,caption,id,media_type,timestamp,comments_count,like_count,permalink,children%7Bmedia_url,id,media_type,permalink%7D&limit='.$num.'&access_token='.sbi_maybe_clean( $connected_account['access_token'] );
				...
				$url = 'https://graph.facebook.com/'.$connected_account['user_id'].'/tags?user_id='.$connected_account['user_id'].'&fields=media_url,caption,id,media_type,comments_count,like_count,permalink,children%7Bmedia_url,id,media_type,permalink%7D&limit='.$num.'&access_token='.sbi_maybe_clean( $connected_account['access_token'] ).$paging;
