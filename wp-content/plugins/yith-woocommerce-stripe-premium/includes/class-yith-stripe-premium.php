		...
		public function resume_subscription( $result, $subscription ) {
			...
			try {
				...
				$date_offset = method_exists( YWSBS_Subscription_Helper(), 'get_payment_due_date_paused_offset' ) ? YWSBS_Subscription_Helper()->get_payment_due_date_paused_offset( $subscription ) : $subscription->get_payment_due_date_paused_offset();
 				// set trial to undefined date, so any payment is triggered from stripe, without cancel subscription
				$gateway->api->update_subscription( $subscription->stripe_customer_id, $subscription->stripe_subscription_id, array(
					'trial_end' => ( $subscription->payment_due_date > current_time( 'timestamp' ) ) ? $subscription->payment_due_date + $date_offset : current_time( 'timestamp' )
				) );
			...
			try {
				...
				if ( $gateway ) {
					...
					YITH_WCStripe()->get_customer()->update_usermeta_info( $customer->metadata->user_id, array_merge(
						array(
							'id'             => $customer->id,
							'cards'          => $customer->sources ? $customer->sources->data : [],
							'default_source' => $token->get_token()
						)
					) );
				}
		...
		public function delete_token_from_stripe( $token_id, $token ) {
			...
			try {
				...
				if ( $gateway ) {
					...
					// trigger action after a card was deleted.
