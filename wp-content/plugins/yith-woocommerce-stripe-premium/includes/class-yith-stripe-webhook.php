	...
	class YITH_WCStripe_Webhook {
		...
		public static function route() {
			...
			if ( ! $callback || ! method_exists( __CLASS__, $callback ) ) {
				...
				self::_sendSuccess( sprintf( __( 'No action to perform with this event (method invoked is: %s.', 'yith-woocommerce-stripe' ), $callback ) );
			}
		...
		protected static function invoice_payment_succeeded( $invoice = false ) {
			...
			// check the domain
			if ( ! isset( $stripe_subscription_line_obj->metadata->instance ) || $stripe_subscription_line_obj->metadata->instance != $gateway->instance ) {
				self::_sendSuccess( 'Instance does not match -> ' . $stripe_subscription_line_obj->metadata->instance . ' : ' . $gateway->instance );
			}
		...
		protected static function customer_subscription_deleted() {
			...
			// check the domain
			if ( ! isset( $stripe_subscription->metadata->instance ) || $stripe_subscription->metadata->instance != $gateway->instance ) {
				self::_sendSuccess( 'Instance does not match -> ' . $stripe_subscription->metadata->instance . ' : ' . $gateway->instance );
			}
